...
def init_cmd(force: bool, use_defaults: bool) -> None:
    ...
    if use_defaults:
        # Using --defaults: enable all skills automatically
        from ..agents.skills_manager import sync_skills_to_working_dir

        click.echo("Enabling all skills by default...")
        sync_skills_to_working_dir(skill_names=None, force=False)
        click.echo("✓ All skills enabled.")
    elif write_config:
    ...
    # Always check and copy MD files if language changed, independent of config
    ...
    if installed_language != current_language or force:
        ...
        if copied > 0:
            ...
            click.echo(f"✓ Copied {copied} md file(s)")
        else:
    ...
    if heartbeat_path.is_file() and not force and not use_defaults:
