class khn {
  constructor() {
    Ut(this, "lsKey");
    Ut(this, "sessionList");
    Ut(this, "fetchPromise", null);
    Ut(this, "lastFetchTime", 0);
    Ut(this, "cacheTimeout", 5e3);
    Ut(this, "sessionCache", new Map);
    Ut(this, "sessionFetchPromises", new Map);
    Ut(this, "sessionCacheTimeout", 5e3);
    this.lsKey = "agent-scope-runtime-webui-sessions", this.sessionList = []
  }
  ...
  async fetchSessionListFromBackend() {
    try {
      const n = (await Xr.listChats()).filter(r => r.id && r.id !== "undefined" && r.id !== "null");
      return this.sessionList = n.map(Phn).reverse(), localStorage.setItem(this.lsKey, JSON.stringify(this.sessionList)), this.lastFetchTime = Date.now(), [...this.sessionList]
    } catch {
  ...
  async removeSession(t) {
    try {
      ...
      const n = t.id;
      return await Xr.deleteChat(n), this.sessionList = this.sessionList.filter(r => r.id !== n), localStorage.setItem(this.lsKey, JSON.stringify(this.sessionList)), this.lastFetchTime = Date.now(), [...this.sessionList]
    } catch {
